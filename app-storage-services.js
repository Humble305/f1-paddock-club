// app-storage-services.js
// 本地存储、签到、日记辅助、头像、API 列表能力
...
function addFavorability(driverId, inc) {
    ...
    showToast(`?? +${inc} 好感度${driver ? ` (${driver.name})` : ''}`, false);
    return true;
}
...
function saveAvatarToLocal(driverId, base64) {
    driverAvatars[driverId] = base64;
    localStorage.setItem('f1_driver_avatars', JSON.stringify(driverAvatars));
}

function getDriverAvatarStyle(driverId) {
    return driverAvatars[driverId] ? `url(${driverAvatars[driverId]})` : null;
}

function renderAvatarOnElement(el, driverId, size = '52px') {
    if (!el) return;
    const driver = window.DRIVERS.find(item => item.id === driverId);
    el.style.width = size;
    ...
    if (avatarBg) {
        el.style.backgroundImage = avatarBg;
        el.style.backgroundSize = 'cover';
        el.style.backgroundPosition = 'center';
        el.innerText = '';
    } else {
        el.style.backgroundImage = '';
        el.style.backgroundColor = window.TEAM_COLORS?.[driver?.team] || '#2a2f3a';
        el.innerText = driver ? driver.avatarLetter : '???';
    }
}
