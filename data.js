...
const ANNOUNCEMENTS = [
    { version: "v4.1.0", content: "🛠️ 聊天体验升级 & 媒体资讯恢复增强\n• 移动端聊天界面继续精修，沉浸式聊天更干净，送礼按钮和输入区布局更协调\n• 消息操作升级为更轻巧的小功能区，支持转发、删除、编辑，转发消息也有了更清晰的特殊气泡样式\n• 编辑自己的消息后，会联动车手回复一起重置并重新生成，聊天逻辑更自然\n• 群聊系统现已更完整：可创建群聊、折叠显示、查看群成员、编辑群资料与群头像，群内角色回复也更像真实多人聊天\n• 角色资料库继续补强，车手说话方式更贴近本人，减少固定梗复读，并补足 F1 历史与 2026 新规知识基线\n• 媒体页已恢复最新资讯联网拉取，支持多源聚合、缓存状态、国内/英文筛选，以及横向胶囊关键词筛选 ✨" },
...
window.defaultTeamStandings = teamStandings.map(item => ({ ...item }));
window.defaultDriverStandings = driverStandings.map(item => ({ ...item }));
...
const ANNOUNCEMENTS_UI = [
    { version: "v4.4.0", content: "AI 好感判定升级 & 约会事件卡上线\n• 聊天好感度判定已从关键词命中升级为 AI 语义判断：系统现在会结合车手人设、关系状态与话题方向，判断你这句话有没有真的说到他心里，不再只是碰巧说中几个词就机械加分\n• 群聊里的好感变化也同步接入这套 AI 判定逻辑，只有真正开口接你话的车手，才会按各自的人设理解你的内容并决定要不要增加好感\n• 约会系统新增「事件卡」玩法：约会过程中会随机遇到一些小插曲，例如差点被认出来、天气突然变化、海边顺手拍照、车库临时来消息，让每场约会都更像真的在经历一个晚上\n• 事件卡现在会以独立弹窗形式触发，不再和约会聊天气泡挤在一起；选项更完整、阅读更清楚，也更贴合当前全局的玻璃感 UI 风格\n• 为了避免节奏太密，约会事件卡加入了触发冷却，不会连续一张接一张地跳出来；处理完一次事件后，会先回到正常约会互动，再在更自然的时机出现下一张\n• 约会事件的选择结果会写进约会记忆摘要，后续生成关系日记时也会一起带上，让这段经历不会只停留在当下" },
...
window.ANNOUNCEMENTS = [
    {
        version: "v4.6.0",
        content: "积分榜资料卡升级 & 全屏铺开\n- 车队积分榜升级成了可交互的围场档案面板：现在鼠标悬浮到不同车队、或在手机上点按不同车队时，右侧都会即时切换到对应车队的生涯资料与风格档案，整体更像实时战情板\n- 车手积分榜也同步重做成了同一套交互结构：每位车手现在都有自己的生涯资料卡，会直接结合本地 2026 赛季数据、当前积分位置、胜场、杆位、领奖台与生涯节点来展示，不再只是静态表格\n- 积分页整体动效与视觉语言全面升级：榜单行加入推进光条、扫光、激活轨迹和切换抬升反馈，资料面板也补上了更强的速度线与工程墙质感，整体更贴近 F1 的冷锐速度感\n- 游戏外层容器现已铺满全屏：桌面端和移动端都会直接吃满视口，保留原有模块结构不重排，只把整套界面完整放大铺开"
    },
    {
        version: "v4.5.0",
        content: "车手主页 & 群聊日记上线\n- 资料卡升级成了更完整的车手主页：现在打开任意车手时，不再只是看一张基础资料卡，而是能直接看到他的最近动态、互动里程碑、收到的礼物和当前关系状态，整个人会更立体，也更像真的在围场里留下痕迹\n- 商城礼物偏好从单一命中改成三档反馈：除了“最爱礼物”会明显打动车手之外，现在还有一层“普通喜欢”的礼物偏好，送对了也会得到更自然的正反馈，让送礼体验不再只是全中或全空\n- 群聊现在也有自己的日记系统了：你可以直接在群聊里生成群聊日记，AI 会参考这个群之前聊过的话题、氛围和熟悉程度来续写后续对话，让群聊不再像每次都从零开始\n- 群聊互动记忆也已经接入上下文：群里之前讨论过什么、谁更活跃、最近是什么气氛，都会更稳定地留在这个群自己的连续体验里"
    },
    ...(window.ANNOUNCEMENTS || []).filter(item => item?.version !== "v4.5.0")
];
window.ANNOUNCEMENTS = [
    {
        version: "v4.5.0",
    ...
    {
        version: "v4.4.0",
        content: "AI 好感判定升级 & 约会事件卡上线\n- 聊天好感度判定已从关键词命中升级为 AI 语义判断，系统会结合车手人设、关系状态与话题方向，判断你这句话有没有真的说到他心里\n- 群聊里的好感变化也同步接入这套 AI 判定逻辑，只有真正开口接你话的车手，才会按各自的人设理解你的内容并决定要不要增加好感\n- 约会系统新增「事件卡」玩法：约会过程中会随机遇到一些小插曲，让每场约会都更像真的在经历一个晚上\n- 事件卡改成独立弹窗触发，并加入了触发冷却；事件结果会写进约会记忆，后续生成关系日记时也会一起带上"
    },
    {
        version: "v4.3.0",
        content: "群聊细化升级 & 联系人结构重做\n- 好感度从泛泛夸奖升级成每位车手的三级话题权重，聊天必须更贴近对方真正感兴趣的方向，关系推进才会更自然\n- 群聊资料页和群内互动体验整体升级，更接近完整 IM 群聊：群设置、成员管理、状态展示和群内接话逻辑都更细了\n- 左侧联系人列表重构，新增独立置顶联系人区域，原本散在各车队分组里的置顶逻辑改成统一固定在最上方\n- 所有车队分组都加入了和群聊一样的折叠收纳能力，联系人区层级更清楚，也更利落"
    },
    ...(window.ANNOUNCEMENTS || []).filter(item => !['v4.6.0', 'v4.5.0', 'v4.4.0', 'v4.3.0', 'v4.2.0'].includes(item?.version))
];

window.ANNOUNCEMENTS = [
    {
        version: "v4.7.0",
        content: "赛历档案扩展 & 赛前预测小游戏上线\n- 赛历页继续升级成更完整的围场档案视图：现在点到任意分站，不只是在看日期和赛道资料，右侧还会展示这条赛道的历史名片、赛道性格、关键区段和周末气压，整体更像一块会动的比赛工程面板\n- 积分排名的数据源已经从页面逻辑里拆出来了：车队积分和车手积分现在走独立数据层，后面不管是手动更新、替换远程 JSON，还是再接自动化数据接口，都不需要重写积分榜界面本身\n- 赛前预测小游戏正式接进赛历模块：你可以在比赛开始前直接预测这一站的杆位、冠军和领奖台，预测结果会跟着当前分站一起显示，不再像额外塞进来的独立小页面\n- 预测提交之后会直接锁定进浏览器本地数据里，等这站完赛后系统会自动按结果结算围场币；比赛周一旦正式开始，周五到周日也会自动关闭预测入口，节奏会更像真的赛前下注窗口"
    },
    {
        version: "v4.6.0",
        content: "积分榜档案升级 & 全屏铺开\n- 车队积分榜升级成了可交互的围场档案面板：现在鼠标悬浮到不同车队、或在手机上点按不同车队时，右侧都会即时切换到对应车队的生涯资料与风格档案，整体更像实时战情墙\n- 车手积分榜也同步重做成了同一套交互结构：每位车手现在都有自己的生涯资料卡，会直接结合本地 2026 赛季数据、当前积分位置、胜场、杆位、领奖台与生涯节点来展示，不再只是静态表格\n- 积分页整体动效与视觉语言全面升级：榜单行加入推进光条、扫光、激活轨迹和切换抬升反馈，资料面板也补上了更强的速度线与工程墙质感，整体更贴近 F1 的冷锐速度感\n- 游戏外层容器现已铺满全屏：桌面端和移动端都会直接吃满视口，保留原有模块结构不重排，只把整套界面完整放大铺开"
    },
    {
        version: "v4.5.0",
        content: "车手主页 & 群聊日记上线\n- 资料卡升级成了更完整的车手主页：现在打开任意车手时，不再只是看一张基础资料卡，而是能直接看到他的最近动态、互动里程碑、收到的礼物和当前关系状态，整个人会更立体，也更像真的在围场里留下痕迹\n- 商城礼物偏好从单一命中改成三档反馈：除了“最爱礼物”会明显打动车手之外，现在还有一层“普通喜欢”的礼物偏好，送对了也会得到更自然的正反馈，让送礼体验不再只是全中或全空\n- 群聊现在也有自己的日记系统了：你可以直接在群聊里生成群聊日记，AI 会参考这个群之前聊过的话题、气氛和熟悉程度来续写后续对话，让群聊不再像每次都从零开始\n- 群聊互动记忆也已经接入上下文：群里之前讨论过什么、谁更活跃、最近是什么气氛，都会更稳定地留在这个群自己的连续体验里"
    }
];
window.ANNOUNCEMENTS = [
    {
        version: "v4.7.0",
        content: "赛历档案扩展 & 赛前预测小游戏上线\n- 赛历页继续升级成更完整的围场档案视图：现在点到任意分站，不只是看日期和赛道资料，还能在右侧直接读这条赛道的历史名片、赛道性格、关键区段和周末气压，整体更像一块会动的比赛工程面板\n- 积分排名的数据源已经从页面逻辑里拆出来了：车队积分和车手积分现在走独立数据层，后面不管是手动更新、替换远程 JSON，还是再接自动化数据接口，都不需要重写积分榜界面本身\n- 赛前预测小游戏正式接进赛历模块：你可以在比赛开始前直接预测这一站的杆位、冠军和领奖台，预测结果会跟着当前分站一起显示，不再像额外塞进来的独立小页面\n- 预测提交之后会直接锁定进浏览器本地数据里，等这站完赛后系统会自动按结果结算围场币；比赛周一旦正式开始，周五到周日也会自动关闭预测入口，节奏会更像真的赛前下注窗口"
    },
    {
        version: "v4.6.0",
        content: "积分榜档案升级 & 全屏铺开\n- 车队积分榜升级成可交互的围场档案面板：现在悬停或点按不同车队时，右侧会即时切换到对应车队的生涯资料与风格档案，整体更像实时战情墙\n- 车手积分榜也同步做成了同一套交互结构：每位车手现在都有自己的生涯资料卡，会结合 2026 赛季积分位置、胜场、杆位、领奖台和生涯节点来展示，不再只是静态表格\n- 积分页整体动效与视觉语气全面升级：列表、资料面板和状态切换都补上了更强的速度线、扫光和推进反馈，整体更贴近 F1 的冷锐速度感\n- 游戏外层容器改成全屏铺开，桌面和移动端都会直接铺满视口，在不重排核心模块的前提下把整体界面完整放大"
    },
    {
        version: "v4.5.0",
        content: "车手主页 & 群聊日记上线\n- 资料卡升级成更完整的车手主页：现在打开任意车手时，不再只是基础信息，而是能直接看到最近动态、互动里程碑、收到的礼物和当前关系状态，整个人会更立体，也更像真的活在围场里\n- 商城礼物偏好从单一命中改成三档反馈：除了“最爱礼物”会明显打动车手之外，现在还加入了一层“普通喜欢”的礼物反馈，让送礼体验不再只有全中或全空\n- 群聊现在也有自己的日记系统了：你可以直接在群聊里生成群聊日记，AI 会参考这个群之前聊过的话题、氛围和熟悉程度来延续后续对话，让群聊不再像每次都从零开始\n- 群聊互动记忆也已经接入上下文，群里之前讨论过什么、谁更活跃、最近是什么气氛，都会更稳定地留在这个群自己的连续体验里"
    }
];
window.ANNOUNCEMENTS = [
    {
    ...
    {
        version: "v4.4.0",
        content: "AI 好感判定升级 & 约会事件卡上线\n- 聊天好感度判定已从关键词命中升级成 AI 语义判断，系统会结合车手人设、关系状态与话题方向，判断你这句话有没有真的说到他心里\n- 群聊里的好感变化也同步接入这套 AI 判定逻辑，只有真正开口接你话的车手，才会按各自的人设理解你的内容并决定要不要增加好感\n- 约会系统新增“事件卡”玩法：约会过程中会随机遇到一些小插曲，让每场约会都更像真的在经历一个晚上\n- 事件卡改成独立弹窗触发，并加入了触发冷却；事件结果会写进约会记忆，后续生成关系日记时也会一起带上"
    },
    {
        version: "v4.3.0",
        content: "群聊细化升级 & 联系人结构重做\n- 好感度从泛夸奖升级成按每位车手的三级话题权重判定，聊天必须更贴近对方真正感兴趣的方向，关系推进才会更自然\n- 群聊资料页和群内互动体验整体升级，更接近完整 IM 群聊：群设置、成员管理、状态展示和群内接话逻辑都更细了\n- 左侧联系人列表重构，新增独立置顶联系人区域，原本散在各车队分组里的置顶逻辑改成统一固定在最上方\n- 所有车队分组都加入了和群聊一样的折叠收纳能力，联系人区层级更清晰，也更利落"
    },
    ...(window.ANNOUNCEMENTS || []).filter(item => !['v4.7.0', 'v4.6.0', 'v4.5.0', 'v4.4.0', 'v4.3.0'].includes(item?.version))
];
