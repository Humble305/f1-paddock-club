...
const ANNOUNCEMENTS = [
    ...
    {
        version: "v4.6.0",
        content: "📊 积分榜交互升级 & 全屏铺开\n• 车队积分榜升级成可交互的围场档案面板：悬停或点按不同车队时，右侧会即时切换对应车队的生涯资料与风格档案\n• 车手积分榜也同步做成了同一套交互结构：每位车手都会结合 2026 赛季数据、当前积分、胜场、杆位与领奖台记录展示完整生涯卡片\n• 积分页整体动效继续强化：速度线、扫光、推进反馈和切换抬升都更完整，观感更贴近 F1 工程墙与战情板\n• 游戏外层容器改成全屏铺开，桌面与移动端都会直接吃满视口，整体显示更完整 🚥"
    },
    {
        version: "v4.5.0",
        content: "🪪 车手主页 & 群聊日记上线\n• 资料卡升级成更完整的车手主页：现在打开任意车手时，不再只是基础信息，而是能直接看到最近动态、互动里程碑、收到的礼物和当前关系状态\n• 商城礼物偏好从单一命中升级成三档反馈：最爱礼物、普通喜欢和普通收下会给出不同强度的回应，送礼体验更自然\n• 群聊现在也有自己的日记系统：可以直接生成群聊日记，AI 会参考群里之前聊过的话题与氛围来延续对话\n• 群聊记忆已经正式接入上下文，群内互动会更连贯，不再像每次都从零开始 📝"
    }
];
let mediaNewsItems = [
