...
function clearSidebarActive() {
    document.querySelectorAll('.sidebar-btn.active').forEach(button => {
...
function openHistoryTodayPage() {
    ...
    if (dateSpan) dateSpan.innerText = `${today.getMonth() + 1}/${today.getDate()}`;

    const events = window.getHistoryEventsByDate?.(monthDayKey) || [];
    const safeEvents = events.length
        ? events
        : [{ year: today.getFullYear(), title: 'No event recorded today', desc: 'The history library does not have an entry for today yet.' }];

    const container = document.getElementById('historyEventsList');
    if (!container) return;
    container.innerHTML = safeEvents.map(event => `
        <div class="history-card">
            <div class="history-year"># ${escapeHtml(String(event.year || ''))}</div>
...
function clearCurrentDriverChatHistory() {
    ...
    const confirmed = confirm(`Clear chat history with ${currentChatDriver.name}?`);
    if (!confirmed) return;
